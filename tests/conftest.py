...
def make_provider(name="test-provider", provider="openai", base_url="https://api.test.com/v1",
                  api_key="sk-test-key", enabled=True, custom_headers=None):
    return ProviderConfig(
        name=name, provider=provider, base_url=base_url,
        api_key=api_key, enabled=enabled, custom_headers=custom_headers or {},
    )
...
def make_model(name="test-model", mode="chain", adapter_names=None, model_names=None):
    ...
    return name, ModelConfig(mode=mode, description=f"Test model {name}", adapters=adapters)


def make_api_key(key="sk-test-admin", name="admin", enabled=True, rate_limit=60,
                 daily_limit=0, allowed_models=None):
    return ApiKeyConfig(
        key=key, name=name, enabled=enabled, rate_limit=rate_limit,
        daily_limit=daily_limit, allowed_models=allowed_models or [],
        created_at="2026-01-01T00:00:00", description="Test key",
    )
...
def make_config(providers=None, models=None, api_keys=None):
    ...
    return GatewayConfig(
        ...
        providers=p, models=model_dict, api_keys=keys,
    )
...
def sample_config():
    ...
    _, m_chain = make_model("chain-model", "chain",
                            adapter_names=["provider-a", "provider-b"],
                            model_names=["upstream-a", "upstream-b"])
    _, m_adapter = make_model("direct-model", "adapter",
                              adapter_names=["provider-a"],
                              model_names=["upstream-a"])
    key = make_api_key()
    return make_config(providers=[p1, p2], models={"chain-model": m_chain, "direct-model": m_adapter}, api_keys=[key])
...
def config_with_expired_key():
    ...
    expired_key = ApiKeyConfig(
        key="sk-expired", name="expired-user", enabled=True,
        expires_at="2020-01-01T00:00:00", created_at="2020-01-01T00:00:00",
    )
...
def config_with_disabled_key():
    ...
    disabled_key = ApiKeyConfig(
        key="sk-disabled", name="disabled-user", enabled=False,
...
def config_with_rate_limit():
    ...
    limited_key = ApiKeyConfig(
        key="sk-limited", name="limited-user", enabled=True,
        rate_limit=2, daily_limit=0, created_at="2026-01-01T00:00:00",
    )
...
def config_with_daily_limit():
    ...
    name, m = make_model()
    limited_key = ApiKeyConfig(
        key="sk-daily-limited", name="daily-limited", enabled=True,
        rate_limit=0, daily_limit=2, created_at="2026-01-01T00:00:00",
    )
    return make_config(providers=[p], models={name: m}, api_keys=[limited_key])
...
def config_with_model_restriction():
    ...
    restricted_key = ApiKeyConfig(
        key="sk-restricted", name="restricted-user", enabled=True,
        allowed_models=["allowed-model"], created_at="2026-01-01T00:00:00",
    )
...
def _create_test_app(config):
    ...
    app.add_middleware(GatewayMiddleware, config=config, active_requests_counter=active_requests)
